<?php
    ...
    class ActiveRecord extends \yii\db\ActiveRecord
    {
        ...
        public function scenarios()
        {
            ...
            $scenarios[self::SCENARIO_REST_INSERT] = $scenarios[self::SCENARIO_DEFAULT];
            $scenarios[self::SCENARIO_REST_UPDATE] = $scenarios[self::SCENARIO_DEFAULT];
         ...
         * @return string
         */
        public static function getAlias($field, $alias = '', $tableName = false)
        {
            $alias = self::getEscapedField($alias);

            if ($tableName === false) {
                return self::getTableField($field).((!empty($alias)) ? ' '.$alias : '');
            } else {
                $field = self::getEscapedField($field);

                return self::getEscapedField($tableName).'.'.$field.((!empty($alias)) ? ' '.$alias : '');
            }
        }
         ...
         * @return string
         */
        public static function tableNameWithAlias($alias = '')
        {
            return static::tableName().' '.self::getEscapedField($alias);
        }
